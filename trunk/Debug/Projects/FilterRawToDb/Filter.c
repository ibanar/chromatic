...
LPWAFILTERFILE __stdcall FilterProc(char *FileName, LPWAFILTERFILE File, LPWALIB WALIBStruct) {
	...
	/* Convert main part */
	if(File_Len > 15) WALIBStruct->WAStringFillHexBytes(File->FileMem, Output_Buffer, 16, 5, 5, File_Len16, 3);
	/* Convert remaining bytes */
	if(File_Remainder != 0) WALIBStruct->WAStringFillHexBytes(File->FileMem + (File_Len - File_Remainder), Output_Buffer_End, File_Remainder, 5, 0, 1, 3);
    	/* Write the header */
    	Output_Buffer_Final = WALIBStruct->WAStringCat("; Created from file: ", FileName);
    	...
    	Output_Buffer_Final = WALIBStruct->WAStringCat(Output_Buffer_Final, "_Size\tequ\t");
    	Output_Buffer_Final = WALIBStruct->WAStringCat(Output_Buffer_Final, WALIBStruct->WAStringDecToString(File_Len));
    	Output_Buffer_Final = WALIBStruct->WAStringCat(Output_Buffer_Final, "\r\n");
    	Output_Buffer_Final = WALIBStruct->WAStringCat(Output_Buffer_Final, FileName_Label);
